...
const CAM_LERP = 8
var FOVVIEW = {"default" : 75 , "sprint" : 80}
...
@onready var sprint__timer = $Sprint__Timer
...
@onready var ray = $Head/Camera3D/OBJ_PICKSYS/RayCast3D
@onready var marker = $Head/Camera3D/OBJ_PICKSYS/Marker3D
@onready var joint_3d = $Head/Camera3D/OBJ_PICKSYS/Joint3D
@onready var rot_obj_node = $Head/Camera3D/OBJ_PICKSYS/TO_rot_obj
...
func _process(delta):
	if speed == sprint_speed and not speed == normal_speed:
		$Head/Camera3D/HUD/check.visible = true
	else:
		$Head/Camera3D/HUD/check.visible = false
	...
	if Input.is_action_just_pressed("sprint") and is_forward_moving:
		sprinting = true
		sprint__timer.start()
		
		
	if sprinting == false:
		camera.fov = lerpf(camera.fov, FOVVIEW["default"], CAM_LERP * delta)
		speed = normal_speed
	if sprinting == true:
		speed = sprint_speed
		camera.fov = lerpf(camera.fov, FOVVIEW["sprint"], CAM_LERP * delta)
...
func _on_sprint__timer_timeout():
	sprinting = false
